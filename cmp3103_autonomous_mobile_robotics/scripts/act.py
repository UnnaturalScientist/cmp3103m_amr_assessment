...
def clbk_laser(msg):
    regions = {
        'fleft': min(min(msg.ranges[0:254]), 10),
        'front':  min(min(msg.ranges[255:381]), 10),
        'fright':  min(min(msg.ranges[382:639]), 10),
    }
...
def take_action(regions):
    ...
    if regions['front'] <= 0.35 and regions['fleft'] <= 0.35 and regions['fright'] <= 0.35:
        state_description = 'case 1 - front and fleft and fright'
        linear_x = 0.0 # stop and 180
        angular_z = -0.3

    elif regions['front'] >= 0.45 and regions['fleft'] >= 0.45 and regions['fright'] <= 0.35:
        state_description = 'case 2 - steer left'
        angular_z = 0.3 # steer left

    elif regions['front'] <= 0.35 and regions['fleft'] >= 0.45 and regions['fright'] <= 0.35:
        state_description = 'case 3 - sharp turn left'
        linear_x = 0.0 # hard turn left
        angular_z = 0.5

    elif regions['front'] >= 0.45 and regions['fleft'] <= 0.35 and regions['fright'] >= 0.35:
        state_description = 'case 4 - steer right'
        angular_z = 0.3 # steer right

    elif regions['front'] <= 0.35 and regions['fleft'] <= 0.35 and regions['fright'] >= 0.45:
        state_description = 'case 5 - sharp turn right'
        linear_x = 0.0 # hard turn right
        angular_z = -0.5
